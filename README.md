...
3. **Batch-free decoder with separated correction paths**: The decoder $f_w(z_n, c_n)$ receives only categorical covariates $c_n$ (site, donor, protocol), **not** the batch indicator $s_n$. This separates batch correction into two structurally different paths: (a) a **constrained additive** path ($b_{g,s_n}$) for per-sample ambient RNA, and (b) a **flexible multiplicative** path through categorical covariates in the decoder for systematic differences between donors, protocols, or sites (e.g. PCR bias, RT efficiency, 10x chemistry versions). In standard scVI, the decoder handles all batch effects through a single flexible path, which can absorb biological variation. The separation is most beneficial when batches have high within-batch cell type diversity (e.g. whole-embryo samples), because the additive background can be cleanly identified as the baseline signal shared across all cells in a batch. Batch-specific dispersion $\theta_{g,s_n}$ provides a third correction path for per-batch variance differences.
