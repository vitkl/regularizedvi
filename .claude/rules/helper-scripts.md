...
- **Canonical planning step ordering**: `draft → /format-plan → /grill-me → /verify-plan-against-inputs → ExitPlanMode`. `/format-plan` runs first so the structure is canonical before grill-me interviews against it; verify runs last on the post-grill plan.
- **Resuming planning in a fresh conversation**: when picking up an existing plan in a new session, start with `/pick-up-plan` instead of step 1. It ingests the primary + sibling plans, explores unfamiliar code, then runs the standard format → grill → verify chain.
- **Delegating a planning subtask to a manual subagent (a fresh VSCode conversation, NOT the `Agent` tool)**: use `/handoff-to-manual-subagent` to package a section of the active plan into a handoff file. The user opens that file in a new VSCode conversation; when that conversation is done, the user invokes `/handoff-to-parent` to write the return payload back. Use base `/handoff` for plain context-budget dumps.
